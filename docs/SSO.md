...
- El portal corporativo (este proyecto) **no tiene autenticación de
  ...
  Auth aquí es `/admin`, con el mismo rol `admins/{uid}` que ya usa Mercue.
- Mercue tiene su propio inicio de sesión (correo/contraseña y Google),
  independiente del portal.
- Portal y Mercue **ya comparten el mismo proyecto Firebase**
...
dominios distintos (Mercue y un futuro Producto 2 corren en orígenes
...
Mercue" / "Entrar a Producto 2") que redirija al flujo de arriba, pero la
